...
写AOF文件是主线程完成的，虽然是在命令执行后写文件不会阻塞当前命令，但因为主线程操作会影响后续的命令执行。
...
<img src="https://tva1.sinaimg.cn/large/0081Kckwgy1glvu0k2tssj30v80hi40t.jpg" alt="截屏2020-12-21 下午10.10.29" style="zoom:50%;" />
...
* 主从数据不一致：主从同步异步执行导致的必然结果

* 读到过期的数据

  如果你使用的是 Redis 3.2 之前的版本，那么，从库在服务读请求时，并不会判断数据是否过期，而是会返回过期数据。在 3.2 版本后，Redis 做了改进，如果读取的数据已经过期了，从库虽然不会删除，但是会返回空值，这就避免了客户端读到过期数据。

  此外EXPIRE 和 PEXPIRE：它们给数据设置的是从命令执行时开始计算的存活时间，这会导致主从的过期时间不一致性，建议使用EXPIREAT 和 PEXPIREAT：它们会直接把数据的过期时间设置为具体的一个时间点。
...
redis事务并不能提供完备的原子性保证，如果命令存在语法性错误那么整个事务不会执行，但是如果没有语法性错误在执行命令时抱错了，那么redis并不会回滚执行成功的命令。redis并不支持事务的回滚，对于DISCARD命令其实是用作放弃事务操作并不是回滚。

WATCH 机制的作用是，在事务执行前，监控一个或多个键的值变化情况，当事务调用 EXEC 命令执行时，WATCH 机制会先检查监控的键是否被其它客户端修改了。如果修改了，就放弃事务执行，避免事务的隔离性被破坏。然后，客户端可以再次执行事务，此时，如果没有并发修改事务数据的操作了，事务就能正常执行，隔离性也得到了保证。

如果不利用watch命令，在exec命令执行前redis并不保证事务的一致性，当前事务的修改对其他客户端可见。

RDB操作是不会在事务执行时进行的，所以利用RDB进行持久化会导致事务执行期间的数据变更丢失。

总的来说Redis可以保证事务的一致性和隔离性，但不能保证事务的持久性和原子性（看情况）。
...
* 将多个操作在Redis中实现成一个操作，也就是单步操作

  Redis使用单线程串行处理客户端的请求操作命令，所以单个Redis命令操作是互斥的不存在并发问题。

  Redis提供了原子操作命令：INCR和DECR

* 把多个操作写到一个Lua脚本中，以原子性方式执行单个Lua脚本

  Redis会把整个Lua脚本作为一个整体执行，保证Lua脚本执行的原子性。Redis提供eval命令执行Lua脚本。

# Redis分布式锁

SETNX和DEL命令执行获取和释放锁的操作。

* 为了防止DEL命令执行失败导致锁释放不了，可以加个过期时间
* 为了防止A客户端上的锁被B客户端释放，可以给锁变量赋个唯一变量
* 为了防止Redis宕机导致锁失效，可以使用Redlock算法（同时向多个Redis实例申请锁）
...
![preview](https://tva1.sinaimg.cn/large/008eGmZEgy1gmiwu4wmmjj310p0kbtc9.jpg)
